...
| `Ctrl+Shift+R`            | 只改選取幀內的 track id(範圍外維持舊號)   |
| `Ctrl+Shift+D`            | 刪掉選取幀內的整條 track                  |
...
- 手動關掉環景模式時,若目前**已有**幀含跨縫框,會先跳出確認對話框(列出幀數與框數,說明存檔後會把這些框依 `[0,1]` 裁掉、越過接縫的那一段無法復原,undo 不記錄模式),按確定才會真的關閉;沒有跨縫框則直接關,不彈。
- 關閉後存檔**會把跨縫框 clamp 掉**,含跨縫框的幀會立刻變成未存(`*`),因為存出去的座標確實會不同。
...
- **track_id**(`沿用` 排第一,也是預設 —— 補錨點才是主力用法):
  - `沿用 #id`:先點該 track 任一個框,之後畫的新框都掛同一個 id。補錨點連畫好幾幀時用這個。還沒點過任何框時它退回自動取號,所以預設選它拿不到錯號碼。候選號碼跟著「最後**主動**碰過的那條軌跡」走 —— 在右側改掉選取框的 `track_id`、或整條軌跡換號(`Ctrl+R`)後都會換成新號,不會停在剛淘汰的舊號;**刪框則不會動到候選**,刪完選取雖然會遞補到隔壁的框,那是 index 位移而不是你指定的軌跡
...
tracker 也會把某一段誤配給別的目標 —— 那一段要拆出來給別的號碼,其餘是對的。`Ctrl+R` 的全域換號會把本來正確的部分一起改壞。

用法:左側幀清單圈出那一段 → 「編輯 → 改選取幀內的軌跡 id…」(`Ctrl+Shift+R`)→ 填舊號與新號 → 確定。

規則:

- 只改**選取範圍內**的框,範圍外維持舊號。與 `Ctrl+R` 是兩個獨立動作:那個是「整條都編錯號」,這個是「只有這一段錯」。前置也不同(那邊先點框,這邊先圈範圍),混在同一個入口只會讓人搞不清該先做哪一步。
- **改完會變成什麼樣子,對話框直接寫出來**,因為人在清單上拉範圍時看不出來:
  - 範圍外還有幾個框留著舊號(這條軌跡會被**拆成兩段**);範圍已涵蓋整條時會說明舊號將從資料集消失。
  - 範圍外已經有幾個框在用新號(改完會**接成同一條**)—— 這正是拿它接斷軌時想確認的事。
  - 哪幾幀改完會同時出現兩個新號(撞號),列出 seq。
- 軌跡身分同樣是 **`(label, track_id)`**,理由同內插補框與斷軌換號。
- 只改 `track_id`,不動 bbox / ppe / label,不新增也不刪除框。
- `Ctrl+Shift+I` 整組復原(與補框、改 id、刪軌跡共用同一個還原點)。

### 刪掉一段軌跡

tracker 常在目標離場後還吐一串幽靈框,或某一段被誤配到別的目標。這種錯誤總是「一整段」,逐幀點框刪在幾百幀的序列上慢到沒人會做。改成:圈出範圍,一次刪完。

用法:左側幀清單用 `Shift` 拉連續範圍(或 `Ctrl` 加點零散的幀)→ 在清單上按 `Delete`,或走「編輯 → 刪除選取幀內的軌跡…」(`Ctrl+Shift+D`)→ 在對話框裡確認/修改 track → 確定。

規則:

- **要刪哪一條由對話框決定,不是背後的隱性記憶**。最後點過的框只當**預填值**,送出去的永遠是對話框裡那一組,所以不會出現「以為刪 #7、其實刪了 #3」。
  - 之所以不能只靠「最後點過的框」:選好範圍後想換目標,得跑去畫布點框,而**點回清單會把剛拉好的範圍清光**(`ExtendedSelection` 的標準行為,點一列就只剩那一列)。對話框裡改號碼不碰清單選取。
  - 之所以是對話框而不是常駐面板:這是偶爾做一次的破壞性操作,常駐面板會一直佔著左欄的高度與寬度 —— 而面板越疊越高會把視窗頂大到超出螢幕(見「視窗尺寸」)。
- **影響範圍即時連動**:改一次號碼就重算一次,按鈕上直接寫「刪除 41 個框」,對話框裡寫出命中幾幀、範圍外還留著幾個框。沒有命中就不讓按下去。
...
右側那一疊面板包在捲動區裡,**內容不會反過來決定視窗大小**。

不包的話,面板的最小高度會直接變成視窗的最小高度(實測 767px,加上選單列與狀態列就要 836px),而 `QMainWindow` 一旦發現視窗比最小高度小就會自己把視窗頂大、**而且只長不縮**。後果是選一個框(`det_panel` 要多顯示三行 bbox 座標)視窗就莫名其妙長高一截,在 150% DPI 縮放的螢幕上(邏輯高度只剩 720)底部整個被推出畫面外。包起來之後最小高度降到 318px,面板要多高都行,塞不下就捲動。

同理:**不要為了省事把常駐面板往左右兩欄疊**。偶爾用一次的操作放對話框(如「刪掉一段軌跡」),那是最省版面的地方。

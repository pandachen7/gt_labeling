...
用法:左側幀清單用 `Shift` 拉連續範圍(或 `Ctrl` 加點零散的幀)→ 確認清單下方「Delete 刪除軌跡」的目標是對的 → `Delete` 鍵、目標欄按 `Enter`、或按鈕,三選一 → 確認。

規則:

- **要刪哪一條寫在清單下方,看得見也改得到**。點畫布上的框會自動填進去(所以「點框 → 圈範圍 → 刪」照樣順),但不必非得那樣做 —— 直接在那裡打號碼也可以。
  - 這一格是**唯一真值**,不是畫面顯示、實際另有一份影子狀態。
  - 之所以不能只靠「最後點過的框」:選好範圍後想換目標,得跑去畫布點框,而**點回清單會把剛拉好的範圍清光**(`ExtendedSelection` 的標準行為,點一列就只剩那一列)。目標能就地改,才不會被逼著重選範圍。
  - 按鈕之所以必要:改完號碼焦點在輸入框上,那時 `Delete` 鍵不會落在清單身上。
- 確認視窗一定寫出 `label #id` 與命中框數,不會默默刪錯條。
